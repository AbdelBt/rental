  ...
  if (!page)
    return (
      <div className="font-sora bg-dark-bg text-cream min-h-screen flex items-center justify-center flex-col gap-4">
        ...
        <Link to="/" className="text-gold no-underline">
          ← Retour à l'accueil
        </Link>
      </div>
    );
  ...
  return (
    <div className="font-sora bg-dark-bg text-cream min-h-screen">
      ...
      <div className="max-w-[900px] mx-auto py-16 px-5 md:px-10 pb-20">
        {page.sections && (
          <div
            className={`grid gap-5 ${isMobile ? "grid-cols-1" : "grid-cols-2"}`}
          >
            {page.sections.map((s, i) => (
              <div
                key={i}
                className="bg-dark border border-white/[0.07] rounded-2xl p-7"
              >
...
function ContactForm() {
  ...
  return sent ? (
    <div className="text-center py-[60px]">
      ...
      <h3 className="font-playfair text-[28px] font-bold mb-3">
        Demande envoyée !
      </h3>
