  ...
  return (
    <div>
      ...
      {editable ? (
        ...
        <div className="input-field text-cream/70 cursor-default select-text">{value || "—"}</div>
      )}
    </div>
  );
}
...
const DOCS = [
  { label: "Permis de conduire",            key: "license",  verifiedKey: "license_verified"  },
  { label: "Carte d'identité — face avant",  key: "id_front", verifiedKey: "id_front_verified" },
  { label: "Carte d'identité — face arrière", key: "id_back",  verifiedKey: "id_back_verified"  },
];

function DocRow({ label, docKey, verifiedKey, authId, customer, onUploaded }) {
    ...
    <div className="flex items-center justify-between py-3 border-b border-white/[0.05] last:border-0 gap-3 flex-wrap">
      <div className="flex items-center gap-3 min-w-0">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
          ...
          : uploaded ? "bg-amber-500/10 text-amber-400"
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${
          ...
          {verified ? "Vérifié" : uploaded ? "En attente" : "Manquant"}
        </span>

        {!verified && (
          <label className={`text-[11px] font-semibold px-2.5 py-1 rounded-lg border cursor-pointer transition-colors ${
            uploading ? "border-white/10 text-cream/30 cursor-not-allowed"
...
export default function ClientProfile() {
  const { client, customer, user } = useClientAuth();
  ...
  const handleDocUploaded = (docKey) =>
    setUploadedThisSession((p) => ({ ...p, [docKey]: true }));

  // Merge session uploads into customer so DocRow shows "En attente" immediately
  const mergedCustomer = customer
    ? {
        ...customer,
        license_uploaded:  customer.license_uploaded  || !!uploadedThisSession.license,
        id_front_uploaded: customer.id_front_uploaded || !!uploadedThisSession.id_front,
        id_back_uploaded:  customer.id_back_uploaded  || !!uploadedThisSession.id_back,
      }
    : null;
  ...
  const missingCount = DOCS.filter((d) => !mergedCustomer?.[`${d.key}_uploaded`]).length;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-cream/45 text-sm">Gérez vos informations personnelles.</p>
      </div>
      ...
      {missingCount > 0 && (
        <div className="flex items-start gap-3 bg-amber-500/[0.07] border border-amber-500/25 rounded-xl p-4">
          <svg className="text-amber-400 shrink-0 mt-0.5" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
          <p className="text-[12px] text-amber-300/80">
            {missingCount === 1 ? "1 document manquant." : `${missingCount} documents manquants.`}
      ...
      <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between mb-1">
          <h2 className="font-semibold text-[14px]">Informations personnelles</h2>
          {!editing ? (
            <button onClick={() => setEditing(true)} className="text-[12px] text-gold hover:text-cream/90 transition-colors">
              Modifier
            </button>
          ) : (
            <div className="flex gap-3">
              <button
                onClick={() => { setEditing(false); setMessage({ text: "", ok: false }); }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Prénom"    value={editing ? form.first_name : (client?.first_name ?? "")} editable={editing} onChange={update("first_name")} />
          <Field label="Nom"       value={editing ? form.last_name  : (client?.last_name  ?? "")} editable={editing} onChange={update("last_name")}  />
          <Field label="Email"     value={client?.email ?? ""} editable={false} type="email" />
          <Field label="Téléphone" value={editing ? form.phone : (client?.phone ?? "")} editable={editing} onChange={update("phone")} />
        </div>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[11px] text-cream/35">{verifiedCount}/{DOCS.length} vérifiés</span>
        </div>
        ...
        {Object.keys(uploadedThisSession).length > 0 && (
          <div className="mt-4 p-3 bg-green-500/[0.08] border border-green-500/20 rounded-xl text-[12px] text-green-400">
            ✓ Document(s) envoyé(s). En attente de vérification par l'équipe Drivo.
          </div>
        )}
      </div>
