...
import { payments, agency } from "../../data/dashboardData";
      ...
      <div className="bg-gradient-to-br from-gold/10 to-gold/5 border border-gold/25 rounded-2xl md:rounded-3xl p-5 md:p-7 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="text-xs md:text-sm text-cream/70 leading-relaxed">
            Le client paie <strong className="text-gold">l'acompte (40%)</strong> à
            Drivo lors de la réservation. Le{" "}
      ...
      >
        ...
        >
          {[
            ...
            ["paid", "Versés", payments.filter((p) => p.status === "paid").length],
            ["pending", "En attente", payments.filter((p) => p.status === "pending").length],
          ].map(([val, label, count]) => (
            <button
              key={val}
              onClick={() => {
            ...
            >
              ...
              <span
                className={`py-0.5 px-2 rounded-full text-xs font-bold ${
                  filter === val ? "bg-gold text-[#0a0a0f]" : "bg-white/10 text-cream/60"
                }`}
              >
                {count}
              </span>
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <div className="text-xs text-cream/45 mt-0.5">{p.date}</div>
                    </div>
                    <span
                  ...
                  <div className="mb-3">
                    <div className="text-sm text-cream/80 mb-1">{p.carName}</div>
                  </div>
        ...
        {filtered.length === 0 && (
          <div
            className={`text-center text-cream/40 ${
              isSmallMobile ? "py-10 px-5 text-sm" : "py-[60px] px-10 text-[15px]"
            }`}
          >
